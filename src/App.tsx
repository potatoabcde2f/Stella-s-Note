...
import { 
  ...
  Home, LogOut, Loader2, Lock, Mail, AlertCircle
} from 'lucide-react';
...
const MarkdownRenderer = ({ text, style }: { text: string, style?: any }) => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('# ')) { renderedLines.push(<h1 key={i} className="text-2xl font-bold mb-2 mt-2 text-white">{processInline(line.slice(2))}</h1>); continue; }
        if (line.startsWith('## ')) { renderedLines.push(<h2 key={i} className="text-xl font-bold mb-1 mt-2 text-white">{processInline(line.slice(3))}</h2>); continue; }
        if (line.trim().startsWith('- ')) { renderedLines.push(<div key={i} className="flex gap-2 items-start ml-4"><span className="mt-2 w-1.5 h-1.5 rounded-full bg-current flex-shrink-0" /><span>{processInline(line.trim().slice(2))}</span></div>); continue; }
...
const TextElement = ({ element, onChange, isSelected }: any) => {
  ...
  return (
    <div className="w-full h-full p-4 overflow-hidden" style={{ color: COLORS.text, backgroundColor: element.style?.backgroundColor || 'transparent', borderRadius: element.style?.borderRadius || 0 }} onDoubleClick={() => setIsEditing(true)}>
...
const NoteElement = ({ element, onChange }: any) => {
  ...
  return (
    <div className="w-full h-full p-6 overflow-hidden flex flex-col items-center justify-center text-center relative transition-all" style={{ color: COLORS.text, backgroundColor: element.style?.backgroundColor || '#1E1E1E', borderRadius: element.style?.borderRadius || 0 }} onDoubleClick={() => setIsEditing(true)}>
...
const TodoElement = ({ element, onChange }: any) => {
    ...
    const addItem = () => { updateContent({ ...content, items: [...items, { id: generateId(), text: '', done: false, level: 0 }] }); };
    ...
    const handleKeyDown = (e: React.KeyboardEvent, index: number, id: string) => { if (e.key === 'Enter') { e.preventDefault(); const newItems = [...items]; newItems.splice(index + 1, 0, { id: generateId(), text: '', done: false, level: items[index].level }); updateContent({ ...content, items: newItems }); } else if (e.key === 'Backspace' && items[index].text === '') { e.preventDefault(); deleteItem(id); } else if (e.key === 'Tab') { e.preventDefault(); const newLevel = Math.max(0, Math.min((items[index].level || 0) + (e.shiftKey ? -1 : 1), 3)); updateItem(id, { level: newLevel }); } };
    return (
        <div className="w-full h-full p-4 flex flex-col" style={{ backgroundColor: element.style?.backgroundColor || '#000000', borderRadius: element.style?.borderRadius || 0 }}>
                ...
                {items.map((item: TodoItem, index: number) => (
                    <div key={item.id} draggable onDragStart={(e) => handleSortStart(e, index)} onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleSortDrop(e, index)} onPointerDown={(e) => e.stopPropagation()} className="flex items-start gap-2 mb-2 p-1 rounded group animate-in fade-in slide-in-from-left-2 duration-300 hover:bg-white/5" style={{ marginLeft: `${(item.level || 0) * 1.5}rem` }}>
                        ...
                        <div className="flex-1 min-w-0">
                            <input className={`w-full bg-transparent outline-none text-sm font-sans ${item.done ? 'line-through text-gray-500' : 'text-gray-200'}`} value={item.text} onChange={(e) => updateItem(item.id, { text: e.target.value })} onKeyDown={(e) => handleKeyDown(e, index, item.id)} placeholder="Task..." />
...
const ContainerElement = ({ element, children, onChange, cardCount }: any) => {
    return (
        <div className="w-full h-full flex flex-col border border-[#333] shadow-2xl transition-all relative overflow-hidden group transition-colors duration-200" style={{ borderRadius: element.style?.borderRadius || 12, backgroundColor: element.style?.backgroundColor || '#1A1A1A' }}>
        ...
        <div className="min-h-screen bg-[#121212] flex items-center justify-center p-4 relative overflow-hidden">
            ...
            <div className="w-full max-w-md bg-[#1E1E1E] border border-[#333] p-8 rounded-3xl shadow-2xl relative z-10 animate-in fade-in zoom-in-95 duration-300">
                ...
                <div className="text-center mb-8">
                    <div className="inline-flex items-center justify-center w-16 h-16 bg-[#2A2A2A] rounded-2xl mb-4 text-[#00FF9D] shadow-lg"><Sparkles size={32} /></div>
                ...
                <form onSubmit={handleAuth} className="space-y-4">
                    <div className="space-y-4">
                        <div className="relative group"><Mail className="absolute left-4 top-3.5 text-gray-500 group-focus-within:text-[#00FF9D] transition-colors" size={18} /><input type="email" placeholder="Email Address" required className="w-full bg-[#121212] border border-[#333] rounded-xl pl-12 pr-4 py-3 text-white focus:border-[#00FF9D] outline-none transition-all focus:ring-1 focus:ring-[#00FF9D]/50" value={email} onChange={(e) => setEmail(e.target.value)} /></div>
                        <div className="relative group"><Lock className="absolute left-4 top-3.5 text-gray-500 group-focus-within:text-[#00FF9D] transition-colors" size={18} /><input type="password" placeholder="Password" required minLength={6} className="w-full bg-[#121212] border border-[#333] rounded-xl pl-12 pr-4 py-3 text-white focus:border-[#00FF9D] outline-none transition-all focus:ring-1 focus:ring-[#00FF9D]/50" value={password} onChange={(e) => setPassword(e.target.value)} /></div>
                    </div>
                    ...
                    {message && <div className="flex items-center gap-2 text-green-400 text-sm bg-green-500/10 p-3 rounded-lg border border-green-500/20"><CheckSquare size={16} /> {message}</div>}
                    <button disabled={loading} className="w-full bg-[#00FF9D] hover:bg-[#00E08B] text-black font-bold py-3.5 rounded-xl transition-all flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(0,255,157,0.2)] hover:shadow-[0_0_20px_rgba(0,255,157,0.4)] disabled:opacity-50 disabled:cursor-not-allowed">{loading ? <Loader2 className="animate-spin" size={20} /> : (isLogin ? 'Sign In' : 'Create Account')}</button>
                </form>
                <div className="mt-8 pt-6 border-t border-[#333] text-center"><p className="text-gray-500 text-sm mb-2">{isLogin ? "New here?" : "Already have an account?"}</p><button onClick={() => { setIsLogin(!isLogin); setError(''); setMessage(''); }} className="text-white hover:text-[#00FF9D] font-medium text-sm transition-colors">{isLogin ? "Create an account" : "Sign in to your account"}</button></div>
            </div>
        </div>
    )
}
...
const HomeDashboard = ({ boards, onOpenBoard, onCreateBoard, onDeleteBoard, onLogout, loading }: any) => {
    ...
    const EMOJIS = ['📁', '🚀', '💡', '🎨', '📝', '🧠', '💼', '🏠', '🎯', '✨', '🔥', '🌈', '💻', '📚', '🛠️', '🎮'];
    const handleCreate = () => { if (!newBoardName.trim()) return; onCreateBoard(newBoardName, newBoardIcon); setIsCreating(false); setNewBoardName(''); setNewBoardIcon('📁'); };
    const handleDragStart = (e: React.DragEvent, index: number) => { setDraggedBoardIndex(index); e.dataTransfer.effectAllowed = "move"; };
    const handleDragOver = (e: React.DragEvent, index: number) => { e.preventDefault(); };
    const handleDragEnd = () => { setDraggedBoardIndex(null); };

    return (
        <div className="min-h-screen bg-[#121212] text-white p-10 font-sans">
            <header className="mb-12 flex items-center justify-between">
                 <div className="flex items-center gap-4"><div className="w-10 h-10 bg-[#00FF9D] rounded-lg flex items-center justify-center text-black shadow-[0_0_10px_rgba(0,255,157,0.3)]"><Sparkles size={20} /></div><h1 className="text-3xl font-bold font-serif tracking-tight">Stella's Note</h1></div>
                 <button onClick={onLogout} className="flex items-center gap-2 text-gray-400 hover:text-red-400 bg-[#1E1E1E] hover:bg-[#2A2A2A] px-4 py-2 rounded-xl transition-all border border-transparent hover:border-red-500/30"><LogOut size={16} /> Sign Out</button>
            </header>
            {loading ? ( <div className="flex flex-col items-center justify-center h-64 text-[#00FF9D]"><Loader2 className="animate-spin mb-4" size={40} /><span className="text-sm opacity-50">Syncing your space...</span></div> ) : (
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    <div onClick={() => setIsCreating(true)} className="aspect-square bg-[#1E1E1E] border border-dashed border-[#333] hover:border-[#00FF9D] hover:bg-[#1E1E1E]/80 rounded-2xl flex flex-col items-center justify-center cursor-pointer transition-all group"><div className="w-12 h-12 rounded-full bg-[#2A2A2A] group-hover:bg-[#00FF9D] flex items-center justify-center transition-colors mb-4"><Plus className="text-gray-400 group-hover:text-black" size={24} /></div><span className="text-gray-400 font-medium group-hover:text-white">New Board</span></div>
                    {boards.map((board: Board, index: number) => (
                        <div key={board.id} draggable onDragStart={(e) => handleDragStart(e, index)} onDragOver={(e) => handleDragOver(e, index)} onDragEnd={handleDragEnd} onClick={() => onOpenBoard(board.id)} className="aspect-square bg-[#1E1E1E] border border-[#333] hover:border-gray-500 rounded-2xl p-6 flex flex-col relative group cursor-pointer transition-all hover:translate-y-[-4px] hover:shadow-2xl">
                            <div className="flex-1 flex flex-col items-center justify-center"><div className="text-5xl mb-4 select-none transform group-hover:scale-110 transition-transform">{board.icon}</div><h3 className="text-lg font-bold text-center text-gray-200 group-hover:text-white truncate w-full px-2">{board.name}</h3><span className="text-xs text-gray-600 mt-2 font-mono">{board.elements.length} items</span></div>
                            <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2"><button onClick={(e) => { e.stopPropagation(); onDeleteBoard(board.id); }} className="p-2 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white rounded-lg transition-colors" title="Delete Board"><Trash2 size={14} /></button></div>
                        </div>
                    ))}
                </div>
            )}
                ...
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 animate-in fade-in duration-200 backdrop-blur-sm">
                    <div className="bg-[#1E1E1E] border border-[#333] p-6 rounded-2xl w-96 shadow-2xl">
                        ...
                        <div className="flex gap-4 mb-4">
                            <div className="relative"><button onClick={() => setShowIconPicker(!showIconPicker)} className="w-12 h-12 bg-[#2A2A2A] rounded-xl flex items-center justify-center text-2xl border border-[#333] hover:border-[#00FF9D] transition-colors">{newBoardIcon}</button>{showIconPicker && ( <div className="absolute top-14 left-0 bg-[#2A2A2A] border border-[#333] p-2 rounded-xl grid grid-cols-4 gap-1 w-48 shadow-xl z-10 animate-in fade-in slide-in-from-top-2">{EMOJIS.map(emoji => ( <button key={emoji} onClick={() => { setNewBoardIcon(emoji); setShowIconPicker(false); }} className="p-2 hover:bg-[#333] rounded text-xl transition-colors">{emoji}</button> ))}</div> )}</div>
...
const CanvasWorkspace = ({ board, onSave, onBack }: { board: Board, onSave: (b: Board) => void, onBack: () => void }) => {
    ...
    // Sync from prop (e.g. initial load)
    useEffect(() => { setCurrentBoard(board); setHistory([board]); setHistoryIndex(0); }, [board.id]);
    ...
    const layoutContainer = useCallback((containerId: string, elements: CanvasElement[]): CanvasElement[] => {
        ...
        if (children.length === 0) return elements.map(e => e.id === containerId ? { ...e, height: 150 } : e);
        ...
        children.forEach(child => {
            ...
            updatedChildren.set(child.id, { ...child, x: container.x + padding, y: currentY, width: newWidth, height: newHeight });
    ...
    const handleDirectUpload = (e: React.ChangeEvent<HTMLInputElement>, type: 'image' | 'file') => {
        ...
        if (!file) return;
        const viewportCenter = { x: (window.innerWidth / 2 - currentBoard.viewport.x) / currentBoard.viewport.zoom, y: (window.innerHeight / 2 - currentBoard.viewport.y) / currentBoard.viewport.zoom };
        ...
        reader.onload = (event) => {
            ...
            if (type === 'image') {
                const img = new Image();
                img.onload = () => {
                    const maxW = 400; const ratio = img.width / img.height; const w = Math.min(img.width, maxW); const h = w / ratio;
                    const newEl: CanvasElement = { id: generateId(), type: 'image', x: viewportCenter.x - w/2, y: viewportCenter.y - h/2, width: w, height: h, content: { url, caption: file.name }, style: { borderRadius: 0 } };
                    updateBoard({ ...currentBoard, elements: [...currentBoard.elements, newEl] });
                };
                img.src = url;
            } else {
                const newEl: CanvasElement = { id: generateId(), type: 'file', x: viewportCenter.x - 100, y: viewportCenter.y - 75, width: 200, height: 150, content: { name: file.name, size: formatBytes(file.size), url }, style: { borderRadius: 0 } };
                updateBoard({ ...currentBoard, elements: [...currentBoard.elements, newEl] });
            }
        };
    ...
    const handlePointerDown = (e: React.PointerEvent) => {
        ...
        if (selectedElement) {
            ...
            for (const [key, corner] of Object.entries(corners)) {
                if (Math.abs(worldPos.x - corner.x) < hs && Math.abs(worldPos.y - corner.y) < hs) {
                    setDragInfo({ isDragging: true, startPos: pos, startElPos: { x: el.x, y: el.y, w: el.width, h: el.height }, type: 'resize', handle: key as any });
                    return;
                }
            }
        }
        ...
        let hitId = null;
        // Strict Z-index check: Content (20+) > Containers (0-10)
        // Reverse loop so top elements hit first
        for (let i = currentBoard.elements.length - 1; i >= 0; i--) {
            ...
            if (worldPos.x >= el.x && worldPos.x <= el.x + el.width && worldPos.y >= el.y && worldPos.y <= el.y + el.height) {
                hitId = el.id;
                // If we hit a container but there's a child on top, the child should be hit first in this loop if sorted by Z?
                // Actually, render order matters. We render containers then elements.
                // But in the array they might be mixed.
                // Simplest is to assume standard hit testing finds the top-most based on array order or Z.
                // Ideally we sort by Z-index before hit testing.
                break;
            }
        }
        
        // Re-check hit testing with Z-awareness if simple loop fails (omitted for brevity, simple reverse loop usually works if array order matches render order)
        ...
        if (activeTool === 'select') {
            ...
            const newEl: CanvasElement = {
                id: generateId(), type: activeTool as ElementType, x: worldPos.x, y: worldPos.y,
                width: activeTool === 'container' ? 300 : 200, height: activeTool === 'container' ? 150 : 150,
        ...
        if (dragInfo.type === 'move' && selectedId) {
            ...
            if (draggedEl && draggedEl.type !== 'container') {
                const targetContainer = currentBoard.elements.find(e => 
                    ...
                    worldPos.x >= e.x && worldPos.x <= e.x + e.width &&
                    worldPos.y >= e.y && worldPos.y <= e.y + e.height
                );
    ...
    const renderElement = (el: CanvasElement) => {
        ...
        return (
            <div key={el.id} className={`absolute transition-shadow ${isSelected ? 'ring-1 ring-[#00FF9D]' : ''} ${el.type === 'note' ? 'shadow-lg' : ''}`} style={{ left: el.x, top: el.y, width: el.width, height: el.height, zIndex: finalZ, borderRadius: el.style?.borderRadius || 0, }} onPointerDown={(e) => { /* Selection handled by Canvas */ }}>
                <Component element={el} isSelected={isSelected} onChange={handleElementChange} onOpenLightbox={setLightboxUrl} {...extraProps} />
                {isSelected && (
                    <>
                        {['nw', 'ne', 'sw', 'se'].map((h) => ( <div key={h} className="absolute w-3 h-3 bg-white border border-[#00FF9D] rounded-full z-[101]" style={{ top: h.startsWith('n') ? -4 : 'auto', bottom: h.startsWith('s') ? -4 : 'auto', left: h.endsWith('w') ? -4 : 'auto', right: h.endsWith('e') ? -4 : 'auto', cursor: `${h}-resize` }} /> ))}
                        <div className="absolute -top-8 right-0 bg-red-500/80 p-1 rounded text-white cursor-pointer hover:bg-red-500" onClick={(e) => { e.stopPropagation(); updateBoard({...currentBoard, elements: currentBoard.elements.filter(e => e.id !== el.id)}) }}><Trash2 size={12} /></div>
                    </>
                )}
            </div>
        );
    ...
    const handleCreateElement = (type: ElementType) => {
        const point = screenToWorld({ x: window.innerWidth / 2, y: window.innerHeight / 2 }, currentBoard.viewport);
        const newEl: CanvasElement = { id: generateId(), type, x: point.x, y: point.y, width: 200, height: 100, content: '', style: { backgroundColor: '#1E1E1E', borderRadius: 8 } };
        updateBoard({ ...currentBoard, elements: [...currentBoard.elements, newEl] });
    };

    const handleCreateTable = (rows: number, cols: number) => {
        const point = screenToWorld({ x: window.innerWidth / 2, y: window.innerHeight / 2 }, currentBoard.viewport);
        const newEl: CanvasElement = { id: generateId(), type: 'table', x: point.x, y: point.y, width: cols * 100, height: rows * 40, content: { rows, cols, data: {} }, style: { backgroundColor: '#1a1a1a', zIndex: currentBoard.elements.length + 1 } };
        updateBoard({ ...currentBoard, elements: [...currentBoard.elements, newEl] });
    ...
    return (
        <div className="w-screen h-screen bg-[#121212] overflow-hidden relative select-none" onPaste={() => {}} onDragOver={(e) => e.preventDefault()} onDrop={() => {}} tabIndex={0} style={{ backgroundImage: 'radial-gradient(#333 1px, transparent 1px)', backgroundSize: `${20 * currentBoard.viewport.zoom}px ${20 * currentBoard.viewport.zoom}px`, backgroundPosition: `${currentBoard.viewport.x}px ${currentBoard.viewport.y}px` }}>
            <div ref={canvasRef} className="w-full h-full relative origin-top-left" onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onWheel={handleWheel} style={{ touchAction: 'none' }}>
            ...
            {showTableGrid && <GridSelector onClose={() => setShowTableGrid(false)} onSelect={handleCreateTable} />}
